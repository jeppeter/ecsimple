...
set ECSIMPLE_RANDOP2=z:\rand.bin
set ECSIMPLE_RANDOP=z:\rust_rand.bin
set ECPRIV_FILE=z:\ecpriv.bin
set BIN_FILE=z:\bin.bin
set SIGN_BIN=z:\rust_sign.bin
REM set ECTYPE=SECP112r1
pushd %CD% && cd %CURDIR% && cargo build --release && .\target\release\ectst.exe insertrand -i %ECSIMPLE_RANDOP2% -o %ECSIMPLE_RANDOP% 32 32  &&  popd
