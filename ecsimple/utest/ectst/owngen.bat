...
pushd %CD% && cd %CURDIR% && cargo build --release && .\target\release\ectst.exe ecgenbase %ECTYPE% %PRIVNUMB% %HASHSIZE% --ecpriv %ECPRIVBIN%  --ecpub %ECPUBBIN% --eccmprtype hybrid 2> %OUTFILE% && python z:\pylib\utils.py -i %OUTFILE% -o %SIMPLEOUT% filterlog rust && popd
SET ECSIMPLE_LEVEL=