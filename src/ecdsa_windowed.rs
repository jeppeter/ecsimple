...
pub fn verify(
    ...
    let u1_point = secp256k1
        .g
        .to_affine()
        .multiply_with_windowed_method(u1, &precomputes);

    precomputes = get_window_precomputes(&public_key, None);

    let u2_point = public_key.multiply_with_windowed_method(u2, &precomputes);
